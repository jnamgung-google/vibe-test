view: uscensus20205yr {
  sql_table_name: `eco-shift-478607-e5.bq1.us-census-2020-5yr` ;;
  ...
  parameter: ethnicity_selector {
    type: unquoted
    allowed_value: { value: "Asian" }
    allowed_value: { value: "Black"}
    allowed_value: { value: "White" }
    allowed_value: { value: "Hispanic"}
    allowed_value: { value: "Indian" }
    allowed_value: { value: "Others"}
  }

  measure: pop_by_ethnicity {
    type: sum
    sql:
    CASE
      WHEN {% parameter ethnicity_selector %} == 'Asian' THEN ${asian_pop}
      WHEN {% parameter ethnicity_selector %} == 'Black' THEN ${black_pop}
      WHEN {% parameter ethnicity_selector %} == 'White' THEN ${white_pop}
      WHEN {% parameter ethnicity_selector %} == 'Hispanic' THEN ${hispanic_pop}
      WHEN {% parameter ethnicity_selector %} == 'Indian' THEN ${amerindian_pop}
      WHEN {% parameter ethnicity_selector %} == 'Others' THEN ${other_race_pop}
      ELSE 0
    END ;;
  }
}
